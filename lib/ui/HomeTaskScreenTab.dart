...
class HomeTaskScreenTab extends StatefulWidget {
  @override
  _HomeTaskScreenTabState createState() => _HomeTaskScreenTabState();
}

class _HomeTaskScreenTabState extends State<HomeTaskScreenTab> {
  bool flag = false;
  ...
  Future getTaskData(DatabaseHelper dbHelper, List<DBTask> value) async {
    ...
    for (DBTask task in value) {
      ...
      DateTime tDate =  getDateTimeDate(task.dueDate);
      DateTime cDate = DateTime.parse( DateFormat("yyyy-MM-dd").format(DateTime.now()) );

      print(tDate.isAfter(cDate));
      print(tDate.isAtSameMomentAs(cDate));
      print(tDate.isAfter(cDate) || tDate.isAtSameMomentAs(cDate));

      //if (COMPLETE_TASK_STATUS == task.status) // Old Code
      if(tDate.isAfter(cDate) || tDate.isAtSameMomentAs(cDate)){
                            ...
                            child: Container(
                              ...
                              constraints: new BoxConstraints(
                                  maxWidth:
                                      MediaQuery.of(context).size.width - 200),
                              child: new Text(
                                values.status==null ?  PENDING : values.status.toString(),
                                style: TextStyle(
                                  fontSize: 18.0,
                                ),
                                textAlign: TextAlign.left,
