...
class PastTaskScreenTab extends StatefulWidget {
  @override
  _PastTaskScreenTabState createState() => _PastTaskScreenTabState();
}

class _PastTaskScreenTabState extends State<PastTaskScreenTab> {
  bool flag = false;
  ...
  Future<List<TaskMember>> _getData() async {
    ...
    _listKey = GlobalKey();
    taskMemberList = [];
      await dbHelper.getDBAllTask().then((value) async {
        if (value != null) {
  ...
  Future getTaskData(DatabaseHelper dbHelper,List<DBTask> value) async
  {
    for (DBTask task in value) {
      ...
      if (task.custom != null && task.custom.isNotEmpty) {
  ...
  Widget _buildItem(BuildContext context, TaskMember values,
    ...
    return SizeTransition(
      ...
      child: Card(
        ...
        child: ListTile(
          onTap: () => {
            // now recurring task not need to mark as accept/reject
           /* Navigator.push(
                context,
                MaterialPageRoute(
